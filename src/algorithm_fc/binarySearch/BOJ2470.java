...
public class BOJ2470 {
    ...
    static void pro() {
        ...
        for (int left = 1; left <= N - 1; left++) {
            ...
            //array bound exception이 발생하지 않도록 범위설정 중요 (candidate-1)
            if(left+1 <= candidate-1 && candidate-1 <= N && Math.abs(A[left]+A[candidate-1]) < best_sum) {
                v1 = A[left];
                v2 = A[candidate-1];
            ...
            //(candidate)
            if(left+1 <= candidate && candidate <= N && Math.abs(A[left]+A[candidate]) < best_sum) {
                v1 = A[left];
                v2 = A[candidate];
